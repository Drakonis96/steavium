...
actor EpicGamesManager {
    ...
    private var crossOverBottleName: String {
        ProcessInfo.processInfo.environment["STEAVIUM_CROSSOVER_BOTTLE_EPIC"] ?? "steavium-epic"
    }
    ...
    init(appHome: URL? = nil) {
        ...
        self.prefixPath = self.appHome.appendingPathComponent("prefixes/epic", isDirectory: true)
        ...
        self.gameProfilesPath = self.settingsPath.appendingPathComponent("epic-game-profiles.json")
        self.runtimeScriptsPath = self.appHome.appendingPathComponent("runtime/scripts", isDirectory: true)
    }

    nonisolated var storeName: String { "Epic Games" }

    func snapshot() -> StoreEnvironment {
        let hardwareProfile = cachedHardwareProfile
        let epicExecutable = locateEpicExecutable()
        return StoreEnvironment(
            appHomePath: appHome.path,
            prefixPath: prefixPath.path,
            logsPath: logsPath.path,
            wine64Path: detectWine64(),
            storeAppInstalled: epicExecutable != nil,
            storeAppExecutablePath: epicExecutable,
            hardwareProfile: hardwareProfile
        )
    }

    func gameLibraryState(forceRefresh: Bool = false) -> GameLibraryState {
        // Epic Games Store does not use Steam-style .acf manifests.
        // Return empty for now; an Epic-specific scanner can be added later.
        ...
        let networkCheck: RuntimePreflightCheck = {
            do {
                ...
                return RuntimePreflightCheck(
                    kind: .network,
                    status: .ok,
                    detailEnglish: "Epic Games servers are reachable.",
                    detailSpanish: "Los servidores de Epic Games son accesibles."
                )
            } catch {
                return RuntimePreflightCheck(
                    kind: .network,
                    status: .failed,
                    detailEnglish: "Epic Games servers are not reachable right now.",
                    detailSpanish: "Los servidores de Epic Games no son accesibles en este momento."
                )
            }
        }()
    ...
    func createDiagnosticsArchive(
        ...
        let liveLogURL = logsPath.appendingPathComponent("epic-live.log")

        var reportLines: [String] = []
        reportLines.append("Steavium Diagnostics (Epic Games)")
        ...
        reportLines.append("Epic Games installed: \(environment.storeAppInstalled)")
        reportLines.append("Epic Games executable: \(environment.storeAppExecutablePath ?? "-")")
        ...
        if fileManager.fileExists(atPath: liveLogURL.path) {
            let destination = diagnosticsFolderURL.appendingPathComponent("epic-live.log")
        ...
        if fileManager.fileExists(atPath: gameProfilesPath.path) {
            let destination = diagnosticsFolderURL.appendingPathComponent("epic-game-profiles.json")
            if fileManager.fileExists(atPath: destination.path) {
                try fileManager.removeItem(at: destination)
            }
    ...
    func setupStore(gameLibraryPath: String?) async throws -> String {
        ...
        let script = try materializeScript(named: "setup_epic.sh")
        let result = try await ShellRunner.runAsync(
            executable: "/bin/bash",
            arguments: [script.path],
    ...
    func launchStoreDetached(
        ...
        let script = try materializeScript(named: "launch_epic.sh")
        let hardwareProfile = cachedHardwareProfile
        ...
        return "Epic Games Launcher launched. Log: \(liveLogURL.path)"
    }

    func stopStoreCompletely() async throws -> String {
        try prepareDirectories()

        let script = try materializeScript(named: "stop_epic.sh")
        let result = try await ShellRunner.runAsync(
            executable: "/bin/bash",
            arguments: [script.path],
        ...
        tell application "System Events"
            ...
            repeat with p in wList
                try
                    ...
                    if pName contains "Epic" or pName contains "EpicGames" or pName contains "wine" or pName contains "Wine" or pName contains "CrossOver" then
    ...
    func wipeStoreData(clearAccountData: Bool, clearLibraryData: Bool) async throws -> String {
        ...
        let script = try materializeScript(named: "wipe_epic_data.sh")
    ...
    private func locateEpicExecutable() -> String? {
        let crossOverBottlePath = self.crossOverBottlePath

        let candidates = [
            crossOverBottlePath.appendingPathComponent("drive_c/Program Files (x86)/Epic Games/Launcher/Portal/Binaries/Win64/EpicGamesLauncher.exe").path,
            crossOverBottlePath.appendingPathComponent("drive_c/Program Files (x86)/Epic Games/Launcher/Portal/Binaries/Win32/EpicGamesLauncher.exe").path,
            prefixPath.appendingPathComponent("drive_c/Program Files (x86)/Epic Games/Launcher/Portal/Binaries/Win64/EpicGamesLauncher.exe").path,
            prefixPath.appendingPathComponent("drive_c/Program Files (x86)/Epic Games/Launcher/Portal/Binaries/Win32/EpicGamesLauncher.exe").path
        ]
